...
void ts3plugin_onConnectStatusChangeEvent(uint64 serverConnectionHandlerID, int newStatus, unsigned int errorNumber) {
	...
	//previousState = previousInputValue;

	//snprintf(debugOutput, 40, "previousState:%d", previousState);
	//OutputDebugString(debugOutput);

	if (newStatus == STATUS_CONNECTED || newStatus == STATUS_DISCONNECTED)
	{
		// TODO: Bugged, to finalize
		// forceFeature(0);
		// forceFeature(255);
	}
